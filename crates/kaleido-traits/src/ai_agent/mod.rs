...
//! # Tool generation
//!
//! The contract also declares tool-generation types
//! ([`ToolGenerationRequest`], [`ToolImplSpec`], [`ToolGenerationResult`])
//! for a future "generate a new tool when existing ones are insufficient"
//! step. The MVP implementation does not generate tools yet — the types are
//! part of the stable contract surface.
...
pub struct PlanResult {
    ...
    ///
    /// Reserved for a future partial-result mode. The MVP executor returns
    /// [`AgentError::ExecutionFailed`] on the first failing step instead of
    /// a partial result, so this field is always `true` when a
...
pub trait AIAgent: Send + Sync + 'static {
    ...
    ///
    /// In the template MVP, `context` is accepted for forward compatibility
    /// but not consumed — the planner consults the live [`ToolRegistry`]
    /// directly, which is the MVP equivalent of auto-populating
    /// `available_tools`. An LLM planner is expected to consume it.
    ///
    /// # Events
    ///
    ...
    ///
    /// Each action in the plan is executed sequentially. Execution is
    /// **fail-fast**: the first failing action stops the run and returns
    /// [`AgentError::ExecutionFailed`] with the failing step index. (The MVP
    /// executor does not return a partial [`PlanResult`] — per-step outcomes
    /// are observable through the `ai_action_executed` events instead.)
    ///
    /// # Errors
    ///
    /// - [`AgentError::EmptyPlan`] — the plan contains no actions.
    /// - [`AgentError::MaxStepsExceeded`] — the plan exceeds the safety limit.
    /// - [`AgentError::NoImageLoaded`] — no image is loaded in the store.
    /// - [`AgentError::ToolNotFound`] — an action references an unknown tool.
    /// - [`AgentError::ExecutionFailed`] — a tool application failed.
    ///
    /// # Events
    ///
    /// Emits `ai_action_executed` after each attempted step.
