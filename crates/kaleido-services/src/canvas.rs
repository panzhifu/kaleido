...
impl CanvasService {
    ...
    pub fn visible_image_rect(&self) -> (u32, u32, u32, u32) {
        let (top_left_x, top_left_y) = self.viewport.screen_to_image(0.0, 0.0);
        let (bottom_right_x, bottom_right_y) =
            self.viewport.screen_to_image(self.screen_width as f32, self.screen_height as f32);

        let x = top_left_x.max(0.0) as u32;
        let y = top_left_y.max(0.0) as u32;
        let width = (bottom_right_x.min(self.image_width as f32) - x as f32).max(0.0) as u32;
        let height = (bottom_right_y.min(self.image_height as f32) - y as f32).max(0.0) as u32;
...
mod tests {
    ...
    fn test_canvas_visible_tile_coords() {
        let canvas = CanvasService::new(256, 256, 1024, 768);
        // 256x256 image = 4 tiles of 128x128, all visible.
