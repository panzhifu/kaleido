...
//! operations and edges represent data flow.  The graph is evaluated
//! lazily by [`GraphExecutor`]: only the tiles needed for the requested
//! output region are computed, and adjacent point-operations are
...
impl OpGraph {
    ...
    pub fn topo_sort(&self) -> Vec<NodeId> {
        ...
        while let Some(id) = queue.pop_front() {
            sorted.push(id);
            for node in &self.nodes {
                if node.inputs.iter().any(|&(src, _)| src == id) {
                    let deg = in_degree.get_mut(&node.id).unwrap();
                    *deg -= 1;
                    if *deg == 0 {
...
impl Op for FusedOp {
    fn formats(&self) -> OpFormats {
        OpFormats {
            input: self.ops.first().unwrap().formats().input.clone(),
            output: self.ops.last().unwrap().formats().output,
        }
    }
...
impl GraphExecutor {
    ...
    ///
    /// `output_roi` restricts computation to the requested region.
    ///
    ...
    pub fn execute(&self, graph: &OpGraph, output_roi: Rect) -> ImageResult<TiledImage> {
        ...
        for &node_id in &topo {
            ...
            // Determine the ROI this node needs to compute.
            let roi = if node_id == output_id {
                output_roi
            } else {
                output_roi
            };
    ...
    fn execute_node_parallel(
        ...
        // If the ROI is smaller than a tile, just execute directly.
        if roi.width <= tile_size && roi.height <= tile_size {
            return node.op.compute_roi(roi, inputs);
        }
        ...
        // For small tile counts, don't bother with parallelism.
        if total_tiles <= 1 {
            return node.op.compute_roi(roi, inputs);
        }
        ...
        for (rect, tile_image) in tiles {
            // Copy the computed region from the tile into the output.
            // The tile's computed pixels are at (rect.x, rect.y).
            for py in 0..rect.height {
                for px in 0..rect.width {
                    let src_px = tile_image.get_pixel(rect.x + px, rect.y + py);
                    output.set_pixel(rect.x + px, rect.y + py, src_px);
                }
            }
        }
