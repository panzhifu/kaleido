...
/// SIMD Hard Light blend — Overlay with the source and backdrop swapped
/// (the source is the "light": it decides between darkening and lightening).
pub fn blend_hard_light_simd(src: u32x8, dst: u32x8) -> u32x8 {
    let src_r = extract_r(src);
    let src_g = extract_g(src);
    let src_b = extract_b(src);
    let src_a = extract_a(src);
    ...
    // Scalar Hard Light is overlay_channel(dst, src) — the source selects
    // the branch, so pass the backdrop as the first (src) argument.
    let r = overlay_channel_simd(dst_r, src_r);
    let g = overlay_channel_simd(dst_g, src_g);
    let b = overlay_channel_simd(dst_b, src_b);
    let a = blend_alpha_simd(src_a, dst_a);
...
///
/// Input pixels are in RGBA8 format packed as u32 (0xAABBGGRR, i.e.
/// `pixel_to_u32(Pixel)` / `Pixel::to_raw().swap_bytes()`).
...
pub fn blend_8_pixels(src: [u32; 8], dst: [u32; 8], mode: BlendModeSimd) -> [u32; 8] {
    ...
    let result = match mode {
        ...
        BlendModeSimd::HardLight => blend_hard_light_simd(src_vec, dst_vec),
...
/// SIMD-optimized blend modes.
///
/// Mirrors the 12 non-HSL variants of [`kaleido_core::BlendMode`]; the four
/// HSL modes are not implemented and map to [`BlendModeSimd::Normal`] via
/// [`BlendModeSimd::from_blend_mode`] (same fallback as the scalar
/// [`super::blend::blend`]).
...
impl BlendModeSimd {
    /// Maps a core [`kaleido_core::BlendMode`] to its SIMD counterpart.
    ///
    /// The four HSL modes are not implemented in the SIMD kernels and fall
    /// back to [`BlendModeSimd::Normal`] — mirroring the scalar
    /// [`super::blend::blend`].
    pub fn from_blend_mode(mode: kaleido_core::BlendMode) -> Self {
        use kaleido_core::BlendMode as M;
        match mode {
            ...
            M::Hue | M::Saturation | M::Color | M::Luminosity => Self::Normal,
        }
    }
}

/// Packs an RGBA8 [`kaleido_core::Pixel`] into the `0xAABBGGRR` u32 lane
/// layout used by the SIMD kernels (a byte swap of [`kaleido_core::Pixel`]`::to_raw`).
#[inline]
pub fn pixel_to_u32(p: kaleido_core::Pixel) -> u32 {
    p.to_raw().swap_bytes()
}

/// Unpacks a `0xAABBGGRR` u32 SIMD lane back into a
/// [`kaleido_core::Pixel`].
#[inline]
pub fn u32_to_pixel(v: u32) -> kaleido_core::Pixel {
    kaleido_core::Pixel::from_raw(v.swap_bytes())
}
...
mod tests {
    ...
    use kaleido_core::{BlendMode as CoreBlendMode, Pixel};

    #[test]
    fn test_bridge_pixel_roundtrip() {
        let px = Pixel::new(10, 20, 30, 255);
        assert_eq!(u32_to_pixel(pixel_to_u32(px)), px);
        // Lane layout is 0xAABBGGRR.
        assert_eq!(pixel_to_u32(px), 0xFF1E140A);
    }

    #[test]
    fn test_bridge_from_blend_mode_mapping() {
        use CoreBlendMode as M;
        assert_eq!(BlendModeSimd::from_blend_mode(M::Normal), BlendModeSimd::Normal);
        assert_eq!(BlendModeSimd::from_blend_mode(M::Multiply), BlendModeSimd::Multiply);
        assert_eq!(BlendModeSimd::from_blend_mode(M::HardLight), BlendModeSimd::HardLight);
        assert_eq!(BlendModeSimd::from_blend_mode(M::SoftLight), BlendModeSimd::SoftLight);
        // HSL modes fall back to Normal (same as the scalar kernel).
        assert_eq!(BlendModeSimd::from_blend_mode(M::Hue), BlendModeSimd::Normal);
        assert_eq!(BlendModeSimd::from_blend_mode(M::Saturation), BlendModeSimd::Normal);
        assert_eq!(BlendModeSimd::from_blend_mode(M::Color), BlendModeSimd::Normal);
        assert_eq!(BlendModeSimd::from_blend_mode(M::Luminosity), BlendModeSimd::Normal);
    }

    #[test]
    fn test_bridge_scalar_simd_agree() {
        use crate::services::render::blend::blend as scalar_blend;

        let pairs = [
            (Pixel::new(255, 0, 0, 255), Pixel::new(0, 255, 0, 255)),
            (Pixel::new(128, 64, 32, 255), Pixel::new(200, 100, 50, 255)),
            (Pixel::new(255, 0, 0, 128), Pixel::new(0, 255, 0, 255)),
            (Pixel::new(10, 20, 30, 64), Pixel::new(40, 50, 60, 128)),
            (Pixel::new(0, 0, 0, 0), Pixel::new(128, 128, 128, 255)),
            (Pixel::new(255, 255, 255, 255), Pixel::new(0, 0, 0, 0)),
            (Pixel::new(200, 200, 200, 200), Pixel::new(100, 100, 100, 100)),
            (Pixel::new(1, 2, 3, 4), Pixel::new(250, 251, 252, 253)),
        ];
        let src: [u32; 8] = pairs.map(|(s, _)| pixel_to_u32(s));
        let dst: [u32; 8] = pairs.map(|(_, d)| pixel_to_u32(d));

        let modes = [
            CoreBlendMode::Normal,
            CoreBlendMode::Multiply,
            CoreBlendMode::Screen,
            CoreBlendMode::Overlay,
            CoreBlendMode::Darken,
            CoreBlendMode::Lighten,
            CoreBlendMode::ColorDodge,
            CoreBlendMode::ColorBurn,
            CoreBlendMode::HardLight,
            CoreBlendMode::SoftLight,
            CoreBlendMode::Difference,
            CoreBlendMode::Exclusion,
            // HSL modes map to Normal on both sides.
            CoreBlendMode::Hue,
            CoreBlendMode::Luminosity,
        ];

        for mode in modes {
            let expected: [u32; 8] = pairs.map(|(s, d)| pixel_to_u32(scalar_blend(mode, s, d)));
            let actual = blend_8_pixels(src, dst, BlendModeSimd::from_blend_mode(mode));
            for i in 0..8 {
                let e = u32_to_pixel(expected[i]);
                let a = u32_to_pixel(actual[i]);
                // Fixed-point vs float kernels: allow ±2 per channel.
                let close = |x: u8, y: u8| (x as i16 - y as i16).abs() <= 2;
                assert!(
                    close(e.r, a.r) && close(e.g, a.g) && close(e.b, a.b) && close(e.a, a.a),
                    "mode {mode:?} lane {i}: scalar {e:?} vs simd {a:?}"
                );
            }
        }
    }
